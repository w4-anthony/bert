...
flags.DEFINE_string("train_data_files", None, "")
flags.DEFINE_string("test_data_files", None, "")
...
class InputExample(object):
  ...
  def __repr__(self):
    return f'("{self.text_a}", {self.label})'
...
def _read_protobuf_ascii_in_utf8(filename, message):
  with open(filename, 'rb') as ifile:
    text_format.Merge(ifile.read(), message)


def read_training_data(filename):
  """Reads a list of files to be used for training."""
  if not filename or not os.path.isfile(filename):
    raise FileNotFoundError(filename)

  training_files = w4_pb2.TrainingFiles()
  _read_protobuf_ascii_in_utf8(filename, training_files)
  dir_name = os.path.dirname(filename)

  samples_list = []
  for samples_filename in training_files.training_samples_filenames:
    samples_per_intent = w4_pb2.TrainingSamples()
    _read_protobuf_ascii_in_utf8(
        os.path.join(dir_name, samples_filename), samples_per_intent)
    samples_list.append(samples_per_intent)

  return samples_list, training_files
...
class DataProcessor(object):
  ...
  def __init__(
      self, train_data_files, test_data_files,
  ...
  def get_train_examples(self):
    """Gets a collection of `InputExample`s for the train set."""
    if self._num_train_samples_per_class > 0:
      samples_per_intent = {}
      for example in self._train_examples:
        samples_per_intent.setdefault(example.label, []).append(example)
...
def create_model_tf_hub(is_training, input_ids, input_mask, segment_ids, labels,
...
def model_fn_builder_tfhub(num_labels, learning_rate, num_train_steps,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, probabilities) = create_model_tf_hub(
...
def main(_):
  ...
  processor = DataProcessor(
    FLAGS.train_data_files, FLAGS.test_data_files,
